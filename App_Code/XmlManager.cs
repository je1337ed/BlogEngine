...
public static class XmlManager
{
    ...
    public static bool ArchivePageViews(string strPath)
    {
        try
        {
            if (System.IO.File.Exists(strPath))
            {
                string extension = (new FileInfo(strPath)).Extension;
                string FilenameSansExtension = Path.GetFileNameWithoutExtension(strPath);
                string filepath = Path.GetDirectoryName(strPath);
                System.IO.File.Move(strPath, filepath + "\\" + FilenameSansExtension + "_" + Generate.NewUniqueID + "_" + extension);
                return true;
            }
            else
            {
    ...
    public static string DownloadPageViews(string Path)
    {
        try
        {
            ...
            if (System.IO.File.Exists(Path))
            {
                ...
                Serializer.Serialize(sw, PageViews);
                sw.Close();
                return sb.ToString();
            }
            else
    ...
    public static List<PageView> GetPageViews(string Path)
    {
        try
        {
            ...
            using (SqlCommand dbcmd = new SqlCommand { CommandType = CommandType.StoredProcedure, CommandText = "GetPageViews" })
            {
